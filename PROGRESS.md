...
## Phase 20 · D-5.1 recap 全文重写 + 多轮校准 + 替换式整合回 D-5.0（2026-05-08 ~ 05-09）

**1. recap 全文重写**（2026-05-08）
- v3 大纲：5 节结构（定位 / 报告 / Neo / 界面 / 全局）/ 把"报告呈现什么"+"Neo 说什么"升为双核心
- Agent fork 写全文 / 16 项 PM 决议落地 / 898 行替代版

**2. 16 项 PM 决议**（2026-05-08）：
- B1 报告 5→6 章（第 6 章条件性存在）
- B2 关联 Activity 集合保留 + 备注
- B3 实质性反思软销（不作锁定门槛）
- B4 6 维画像 T1 锁定 + 第 6 章简单提及变化
- B5 跨课程记忆按层级 + 按需（取消每章必引用）
- B-9 时序解耦（报告生命周期 vs 学员体验生命周期 两条独立时序线 / 重大底层重构）
- A-3 术语严格区分（报告状态 vs 学员体验状态）
- B-10 学员反驳报告 Neo 举证回应（不当应声虫 / 不改报告）
- B-12+C-7 报告与 Neo 是两个不同主体（不是 Neo 双层语气）
- B-7 第 5 章便签直接生成 2-3 条 / 不需排序
- B-8 任务卡片 Walk-through 第 5 章逐条推
- B-6 红点回访态系统判定（不需 Neo 主观）
- C-1 报告体量保留（首聊仪式感）
- C-2 永久锁定保留 / 不开重新触发
- C-3 第 4 章画像锁定 / 不调
- C-4 锁定态沿用大厅设计 / 不加引导

**3. 5 角色自主 debate**（2026-05-08 ~ 09）
- 4 维 fan-out（PM / Tech Lead / 文档架构师 / 新读者）+ 对抗审查 + 主持人综合决议
- 必改 7 / 建议改 4 / 暂不改 3 / 争议待 PM 5
- 关键修订：30 秒看懂 + 11 项 P0 + ASCII 映射图 + vibeWriting 误读修正

**4. 后续校准**（2026-05-08 ~ 09）：
- 关联 Activity 集合不含 recap（仅 lecture / practice）
- 第 6 章触发条件简化（学员重新进入关联 Activity 复习 / 删 6 维画像变化触发）
- 6 维画像范围收紧（仅第 4 章 T1 / 第 6 章不再呈现）
- 第 3 章 A 认知转化基线明确（T0/T1 时间锚点 + 各维度数据源）

**5. 跨 spec 补充决策（4 项）**（2026-05-08）：
- 默认密码（平台级 / admin 可查不可改）
- TTS 口播范围（大厅+Ora 无 / lecture+practice+recap 有 / AI 语速仅在有 TTS 页）
- Neo 头像 GIF（本次实现 / 替换呼吸占位 / 场域 Neo 自管理）
- recap 抽屉单 Tab（仅 Project / 删报告锚点 Tab）

**6. UI demo 拉入 + 重构跟进**（2026-05-09）：
- 24.63 MB / 9 HTML demo（学员端 4 + 管理端 4 + 登录页）/ 4 演示账号 / 自包含双击启动
- demo recap 重构（recap.html + recap-app.js + recap-mock.js / 5 块→4 块 / 抽屉单 Tab / 任务卡 800ms 错峰整批）

**7. demo vs spec diff 文档**（2026-05-09）：
- 1927 行 v1（145+ 项 diff / 10 项 P0）→ 5 角色自主 debate 优化为 2023 行 v2（11 项 P0 / 8 项争议待 PM）
- demo 重构后 v3 重审：Walk-through 4 步 / 抽屉单 Tab / 任务卡 800ms / 6 维 prev-current 双层 等

**8. recap 替换式整合回 D-5.0**（2026-05-09）：
- D-5.1/05-recap.md（898 行）→ 替换 D-5.0/spec/05-learner/05-recap【已审阅】.md（923 行）
- D-5.1 副本删除 / D-5.0 现是当前完整基线（recap 已含 D-5.1 整合内容）

**关键产出**：
- D-5.0 spec/05-learner/05-recap【已审阅】.md（923 行 / 含 16 项 PM 决议 + debate 修订）
- D-5.0/spec/00-glossary.md 同步更新（关联 Activity / T0/T1 / 红点 等术语）

---

## Phase 21 · 文档结构重组 · D-5.1 → D-5.0 spec 补充区（2026-05-09）

**核心重组**：D-5.1 增量目录功能演化 → 装跨 spec 决议 + UI demo 的"补充区"性质 → 重命名 + 重构。

**1. 目录改名**：`product-V-0.2.0-D-5.1/` → `product-V-0.2.0-D-5.0-supplements/`
- 原"局部超越"语义（recap.md 替代）转为"补充支持"语义
- 改名后职责更清晰

**2. 内容驱动合并**：
- `cross-spec-decisions-2026-05-08.md`（150 行）+ `demo-vs-spec-diff-2026-05-09.md`（2023 行）
- → 合并为 `D-5.0-spec-supplements-2026-05-09.md`（341 行 / 21 项决议）
- 决议类型：4 项跨 spec + 13 项学员端各场域 + 4 项管理端
- 合并过程：DM 一题一拍 / 区分"按 spec 实现 / 按 demo 升 spec / 不记录 / 待新设计"
- 两源文档合并后**已删除**

**3. 工作档移动**：
- `demo-vs-spec-diff-debate-log-2026-05-09.md`（375 行）→ 搬到 `02-temp/`（过程文档定位）
- `naming-convention.md` → 删除（不再单独维护命名规则）

**4. 文档级联同步**：
- 根 README.md / 00-public/README.md / D-5.0/README.md / supplements/README.md / CLAUDE.md / memory（5 文件）全部对齐 D-5.0-supplements
- 历史引用保留（描述已发生事件正确）/ 前瞻规则改"补充区不另起 glossary" 等

**5. demo 重新拉取**（2026-05-09 下午）：
- 从 `D:\01-云学堂\AI TUTOR\20260430睿学项目UI\demo` 同步最新版（112 文件 / +2 / 24.63 MB）
- recap 重构跟进（recap.html / recap-app.js / recap-mock.js）

**关键产出**：
- D-5.0 spec 补充区目录（`product-V-0.2.0-D-5.0-supplements/`）：
  - D-5.0-spec-supplements-2026-05-09.md（341 行 / 21 项决议综合文档）
  - demo/（24.63 MB / 9 HTML / 4 演示账号）
  - README.md（补充区导览）
- 02-temp/demo-vs-spec-diff-debate-log-2026-05-09.md（375 行 / 5 角色辩论记录）
- 顶层文档 + memory 全量对齐

**约束更新**：
- 核心约束 #6 spec 改动方式：D-5.1 增量目录 → D-5.0 spec 补充区 / 整合方式两种（替换式 / 保留为补充）
- 文档版本体系：D-5.1 → D-5.0-supplements
- 项目状态：D-5.0 是当前基线 + 补充区持续迭代
...
**当前状态（2026-05-09）**：
- ✅ D-5.0 是当前完整基线（22 文件 / recap 已含 D-5.1 整合内容 / 923 行）
- 🔄 D-5.0 spec 补充区持续迭代（综合决议文档 + UI demo）
- ⏳ 暂不进入 D-6.0

**待 DM 决定**：
- 综合决议文档 21 项中部分项整合时机（替换式回写 D-5.0 / 或保留为补充）
- D-5.0-spec-supplements-2026-05-09.md § 5 待 PM 决议项（demo vs spec 8 项争议）
- demo 修齐 spec 路径（修 demo 还是改 spec / 双向修齐）

**未来 · 平台 / Agent 构建（补充区封板后启动）**：
- 按 D-5.0 + D-5.0-supplements + 底层建设期 know-how 输出工程接口文档
- 选定开发栈
- 试点客户 onboarding（60 家天使用户中 3-5 家先行）

**待 DM 操作**：
- GitHub 同步（2026-05-09 启动）：加 github remote = `https://github.com/Damion-Wang/NeoLearningProgramHub.git` / 保留内网 GitLab origin / 双 remote 模式
