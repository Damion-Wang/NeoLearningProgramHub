...
学员端是学员与 NeoLearning 平台的全部交互入口——以 **Neo 1v1 陪伴** 为核心，贯穿项目周期内的学习与项目结营后的剩余服务期。学员端的设计总尺度 = "**它让 Neo 更像 1v1 老师吗？**"（详见 § 2 Neo 第一性原理）。
...
> - **§ 4.2 学员端** = 端口架构视角（用户 / AI / 入口）/ **§ 9（本章）** = 旅程 + 场景细节视角（§ 4.2 的展开）/ **§ 6** = 项目生命周期视角（学员端模块在 § 6 各阶段如何被使用）

#### 9.1.2 学员端模块清单

| 层级 | 模块 | 说明 |
|------|------|------|
| 中枢层 | 大厅（§ 9.2）| Neo 辅导（对话）+ 看板（4 区域 / 含**学习成果区 = 4 库**：我的工具 / 我的报告 / 我的高光 / 我的笔记）+ 笔记悬浮球 |
